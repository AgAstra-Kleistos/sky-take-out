...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderSubmitVO submitOrder(OrdersSubmitDTO ordersSubmitDTO) {
        ...
        for (ShoppingCart cart : shoppingCartList) {
            ...
            orderDetail.setOrderId(orders.getId());  //设置当前订单明细相关的订单id，在添加订单时有返回的订单id
            orderDetailList.add(orderDetail);  //批量插入效率更高
        }
    ...
    public PageResult pageQueryUser(int pageNum, int pageSize, Integer status) {
        ...
        //此处使用DTO转换类型：扩宽mapper层分页查询接口
        OrdersPageQueryDTO ordersPageQueryDTO = new OrdersPageQueryDTO();
        ordersPageQueryDTO.setUserId(userId);
        ordersPageQueryDTO.setStatus(status);

        Page<Orders> page = orderMapper.pageQuery(ordersPageQueryDTO);
        List<OrderVO> list = getOrderVOList(page); //使用List类型承接Detail数据

        /*//查询订单明细,并分装OrderVO进行响应
        if (page != null && page.getTotal() != 0) {
            ...
            for (Orders orders : page) {
                ...
                List<OrderDetail> orderDetails = orderDetailMapper.getByOrderId(orders.getId());
                OrderVO orderVO = new OrderVO();
                BeanUtils.copyProperties(orders, orderVO);  //封装orders数据
                orderVO.setOrderDetailList(orderDetails);  //封装orderDetail数据
                list.add(orderVO);  //加入list集合
            }
